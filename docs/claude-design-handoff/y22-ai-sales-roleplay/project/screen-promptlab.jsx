    ...
    <div style={{ padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-dim)', marginTop: 4 }}>Versioned prompts · evaluated against the golden objection set (240 turns) · regression-checked vs. last release.</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
        {PROMPT_VERSIONS.map((p, i) => (
          <div key={p.ver} className={`prompt-col ${p.selected ? 'selected' : ''}`}>
            ...
            <div className="body-code">{p.body}</div>
